...
/*
 * Keep the low-overhead call counters, but do not time these dynarec paths.
 * verify_dirty and the linker/address helpers can run extremely often when a
 * game repeatedly dirties compiled-code pages, so performance-counter calls
 * here can materially change the workload being measured.
 */
...
#define ROLLBACK_COUNT_RETURN_PTR(count_field, value) \
  do { \
...
void *dynamic_linker(void * src, u_int vaddr)
{
  ...
  if(head!=NULL){
    ...
    add_link(vaddr, add_pointer(src_rw,head->addr));
#endif
    ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_count,
      (void*)(((intptr_t)head->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  }
#endif

  struct ll_entry **ht_bin=hash_table[((vaddr>>16)^vaddr)&0xFFFF];
  if(ht_bin[0]&&ht_bin[0]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_count,
    (void *)(((intptr_t)ht_bin[0]->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  if(ht_bin[1]&&ht_bin[1]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_count,
  ...
  if(r==0) ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_count, dynamic_linker(src,vaddr));
  ...
  ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_count, get_addr_ht(r4300->new_dynarec_hot_state.pcaddr));
}
...
void *dynamic_linker_ds(void * src, u_int vaddr)
{
  ...
  if(head!=NULL){
    ...
    add_link(vaddr, add_pointer(src_rw,head->addr));
#endif
    ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_ds_count,
      (void*)(((intptr_t)head->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  }
#endif

  struct ll_entry **ht_bin=hash_table[((vaddr>>16)^vaddr)&0xFFFF];
  if(ht_bin[0]&&ht_bin[0]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_ds_count,
    (void *)(((intptr_t)ht_bin[0]->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  if(ht_bin[1]&&ht_bin[1]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_ds_count,
    (void *)(((intptr_t)ht_bin[1]->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  ...
  if(r==0) ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_ds_count, dynamic_linker_ds(src,vaddr));
  // Execute in unmapped page, generate pagefault execption
  assert(r4300->cp0.tlb.LUT_r[(vaddr&~1) >> 12] == 0);
  assert((intptr_t)r4300->new_dynarec_hot_state.memory_map[(vaddr&~1) >> 12] < 0);
  r4300->delay_slot = vaddr&1;
  TLB_refill_exception(r4300, vaddr&~1, 2);
  ROLLBACK_COUNT_RETURN_PTR(dynamic_linker_ds_count, get_addr_ht(r4300->new_dynarec_hot_state.pcaddr));
}
...
void *get_addr(u_int vaddr)
{
  ...
  if(head!=NULL){
    ht_bin[1]=ht_bin[0];
    ht_bin[0]=head;
    ROLLBACK_COUNT_RETURN_PTR(get_addr_count,
      (void*)(((intptr_t)head->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  }
  ...
  int r=new_recompile_block(vaddr);
  if(r==0) ROLLBACK_COUNT_RETURN_PTR(get_addr_count, get_addr(vaddr));
  // Execute in unmapped page, generate pagefault execption
  assert(r4300->cp0.tlb.LUT_r[(vaddr&~1) >> 12] == 0);
  assert((intptr_t)r4300->new_dynarec_hot_state.memory_map[(vaddr&~1) >> 12] < 0);
  r4300->delay_slot = vaddr&1;
  TLB_refill_exception(r4300, vaddr&~1, 2);
  ROLLBACK_COUNT_RETURN_PTR(get_addr_count, get_addr_ht(r4300->new_dynarec_hot_state.pcaddr));
}
...
{
  struct ll_entry **ht_bin=hash_table[((vaddr>>16)^vaddr)&0xFFFF];
  if(ht_bin[0]&&ht_bin[0]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(get_addr_ht_count,
    (void *)(((intptr_t)ht_bin[0]->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  if(ht_bin[1]&&ht_bin[1]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(get_addr_ht_count,
    (void *)(((intptr_t)ht_bin[1]->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  ROLLBACK_COUNT_RETURN_PTR(get_addr_ht_count, get_addr(vaddr));
}

void *get_addr_32(u_int vaddr,u_int flags)
{
  struct ll_entry **ht_bin=hash_table[((vaddr>>16)^vaddr)&0xFFFF];
  if(ht_bin[0]&&ht_bin[0]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(get_addr_32_count,
    (void *)(((intptr_t)ht_bin[0]->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  if(ht_bin[1]&&ht_bin[1]->vaddr==vaddr) ROLLBACK_COUNT_RETURN_PTR(get_addr_32_count,
    (void *)(((intptr_t)ht_bin[1]->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  ...
  if(head!=NULL){
    ...
    ROLLBACK_COUNT_RETURN_PTR(get_addr_32_count,
      (void*)(((intptr_t)head->addr-(intptr_t)base_addr)+(intptr_t)base_addr_rx));
  }
  ...
  int r=new_recompile_block(vaddr);
  if(r==0) ROLLBACK_COUNT_RETURN_PTR(get_addr_32_count, get_addr(vaddr));
  // Execute in unmapped page, generate pagefault execption
  assert(r4300->cp0.tlb.LUT_r[(vaddr&~1) >> 12] == 0);
  assert((intptr_t)r4300->new_dynarec_hot_state.memory_map[(vaddr&~1) >> 12] < 0);
  r4300->delay_slot = vaddr&1;
  TLB_refill_exception(r4300, vaddr&~1, 2);
  ROLLBACK_COUNT_RETURN_PTR(get_addr_32_count, get_addr_ht(r4300->new_dynarec_hot_state.pcaddr));
}
